<?php
include_once 'class/remisionero.php';
...
if ($accion=="modificar") {
	$idremisionero=$_POST["idremisionero"];
        ...
        $remisionero = new remisionero();
        $result = $remisionero->update_remisionero($conn, $idremisionero, $serie1,$serie2,$autorizacion,$inicio,$fin,$fecha_caducidad);
	
        if ($result)
        {
            $mensaje="Los datos del remisionero han sido actualizados correctamente";
            $validacion=0;
        }
        else
        {
            $mensaje = "<span style='color:#f8f8ff '><img src='../img/error_icon.png'>   El CODIGO ya existe, ERROR al ingresar el remisionero</span>";
            $validacion=1;
        }
	$cabecera1="Inicio >> GUIA DE REMISION &gt;&gt; Actualizar datos ";
	$cabecera2="ACTUALIzar DATOS de GUIA DE REMISION ";
}
