<?php
	...
	$aColumns = array('id_producto','codigo', 'nombre', 'stock','consignacion', 'costo', 'pvp',  'proveedor','grupo','subgrupo','gasto','composicion','aplicacion' );
        $aColumns_Aux=array('a.id_producto','a.codigo', 'a.nombre', 'a.stock','a.stock_consignacion', 'a.costo', 'a.pvp', 'a.proveedor','g.nombre','s.nombre','a.gasto', 'a.composicion','a.aplicacion');
		...
		for ( $i=0 ; $i<count($aColumns) ; $i++ )
		{
			...
			else
			{
				...
				if($aColumns[$i]=="gasto")
				{
				  ...
				  $sOutput .= '"'.str_replace('"', '\"', $gasto).'",';
				}
				else 
				{
					if (($aColumns[$i] == "costo")||($aColumns[$i] == "pvp"))
					{
						$sOutput .= '"'.str_replace('"', '\"', $aRow[ $aColumns[$i] ]).'",';
					}
					else 
