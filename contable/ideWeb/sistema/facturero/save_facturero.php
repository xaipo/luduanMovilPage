...
if ($accion=="modificar") {
	$idfacturero=$_POST["idfacturero"];
        ...
        $result = $facturero->update_facturero($conn, $idfacturero, $serie1,$serie2,$autorizacion,$inicio,$fin,$fecha_caducidad);
	
        if ($result)
        {
            $mensaje="Los datos del facturero han sido actualizados correctamente";
            $validacion=0;
        }
        else
        {
            $mensaje = "<span style='color:#f8f8ff '><img src='../img/error_icon.png'>   El CODIGO ya existe, ERROR al ingresar el facturero</span>";
            $validacion=1;
        }
	$cabecera1="Inicio >> factureros &gt;&gt; Actualizar facturero ";
	$cabecera2="ACTUALIzar DATOS FACTURERO ";
}
