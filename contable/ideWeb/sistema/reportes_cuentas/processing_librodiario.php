...
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
	...
	$sQuery = "
		SELECT SQL_CALC_FOUND_ROWS a.id_factura as id_factura, a.id_cliente as id_cliente, a.tipodocumento as tipodocumento, a.fecha as fecha,b.nombre as nombre,  a.id_banco as id_banco, a.total as total
                FROM formapago b  INNER JOIN librodiario a ON b.id_formapago=a.id_formapago INNER JOIN facturasp fp ON a.id_factura=fp.id_facturap  
                WHERE (a.fecha BETWEEN '$fechainicio' AND '$fechafin') AND (a.tipodocumento=1) AND (fp.cuenta='$cuenta') 
                $sWhere
		$sOrder
		$sLimit
	";
