<?php
	...
	$aColumns = array('id_transferencia', 'fecha');
       
	/* Indexed column (used for fast and accurate table cardinality) */
	$sIndexColumn = "id_transferencia";
	 ...
	 */        
	$sQuery = "
		SELECT SQL_CALC_FOUND_ROWS id_transferencia, fecha  
		FROM transferencia WHERE (borrado = 0)
                $sWhere
		$sOrder
		$sLimit
	";
	...
	while ( $aRow = mysql_fetch_array( $rResult ) )
	{
		...
		for ( $i=0 ; $i<count($aColumns) ; $i++ )
		{
			if ( $aColumns[$i] == "id_transferencia" )
			{
		 ...
		 */
                ...
                $sOutput .= '"'.str_replace('"', '\"', "<a href='#'><img src='../img/ver.png' border='0' width='16' height='16' border='1' title='ver' onClick='ver_transferencia(".$code_aux.")' onMouseOver='style.cursor=cursor'></a>").'",';
