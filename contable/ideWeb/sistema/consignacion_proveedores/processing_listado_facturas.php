<?php
	...
	$aColumns = array('id_facturap', 'fecha', 'codigo_factura', 'empresa',  'totalfactura' );
        $aColumnsAux=array( 'a.fecha', 'a.codigo_factura', 'b.empresa', 'a.totalfactura');
	...
	$sQuery = "
		SELECT SQL_CALC_FOUND_ROWS a.id_facturap as id_facturap, a.codigo_factura as codigo_factura, b.empresa as empresa, a.totalfactura as totalfactura, a.fecha as fecha 
		 FROM   facturasp_consig a INNER JOIN proveedor b ON a.id_proveedor=b.id_proveedor WHERE (a.anulado = 0)
                $sWhere
		$sOrder
		$sLimit
	";
