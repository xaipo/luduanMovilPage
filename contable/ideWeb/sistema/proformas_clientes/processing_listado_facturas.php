<?php
	...
	$aColumns = array('id_factura', 'codigo_factura', 'nombre', 'totalfactura', 'fecha','credito', 'plazo' );
        $aColumnsAux=array('a.codigo_factura', 'b.nombre', 'a.totalfactura', 'a.fecha', 'a.credito', 'a.plazo');
	...
	$sQuery = "
		SELECT SQL_CALC_FOUND_ROWS a.id_factura as id_factura, a.codigo_factura as codigo_factura,
                         b.nombre as nombre, a.totalfactura as totalfactura, a.fecha as fecha, (if(a.credito=0,'NO','SI'))as credito,
                         (a.plazo*30) as plazo
		FROM   proformas a INNER JOIN cliente b ON a.id_cliente=b.id_cliente WHERE (anulado = 0)
                $sWhere
		$sOrder
		$sLimit
	";
