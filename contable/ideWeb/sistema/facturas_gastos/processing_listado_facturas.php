<?php
	...
	$aColumns = array('id_facturap', 'fecha', 'codigo_factura', 'empresa',  'totalfactura','estado','retencion' );
        $aColumnsAux=array( 'a.fecha', 'a.codigo_factura', 'b.empresa', 'a.totalfactura','a.estado');
	...
	$sQuery = "
		...
		FROM proveedor b INNER JOIN facturasp a ON b.id_proveedor=a.id_proveedor INNER JOIN cuenta c ON a.cuenta=c.id_cuenta WHERE (a.anulado = 0) AND (c.gasto = 1) 
                $sWhere
		$sOrder
		$sLimit
	";
	...
	while ( $aRow = mysql_fetch_array( $rResult ) )
	{
		 ...
		 */
                ...
                if($retencion_aux==1)
                {
                    $query_ret="SELECT COUNT(id_retencion) as contador FROM retencion where id_factura=$code_aux";
                    ...
                    {
                        $sOutput .= '"'.str_replace('"', '\"', "<a href='#'><img src='../img/botonretencion_existente.gif' border='0' width='16' height='16' border='1' title='ver' onClick='retencion(".$code_aux.")' onMouseOver='style.cursor=cursor'></a>").'",';
                    }
                }
                else {
                     $sOutput .= '"'.str_replace('"', '\"', "---").'",';
